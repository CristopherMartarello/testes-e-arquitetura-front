...
describe('SearchPromptUseCase', () => {
  ...
  it('should filter prompts list by given term', async () => {
    const useCase = new SearchPromptsUseCase(repository);
  ...
  it('should apply trim when searching term with blank spaces and return the complete prompts list', async () => {
  ...
  it('should search term with blank spaces, applying trim', async () => {
  ...
  it('should deal with undefined or null term and return the complete prompts list', async () => {
    const findMany = jest.fn().mockResolvedValue(input);
    const searchMany = jest.fn().mockResolvedValue([]);
    const repositoryWithSpies: PromptRepository = {
      ...repository,
